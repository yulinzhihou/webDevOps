...
if [[ -e /data/wwwroot ]] && [[ $2 -eq 1 ]]; then
    ...
    cd /data/wwwroot/
    echo 'tar the project directory and the sql file to package please hold a minutes ........'
    tar zcf ${TAR} homemoji_vn1_beta
    echo "tar $TAR of $1 successfully" >> /data/aslog/`date "+%Y%m%d"`/$1.log
    sleep 5
    #进入ERP项目目录
    if [[ ! -e /data/wwwroot/homemoji_vn1_beta ]];then
        echo "the project directory is not exists in this server, now init the project........."
        git clone git@codehub-cn-south-1.devcloud.huaweicloud.com:homemoji_vn100001/homemoji_vn1.git homemoji_vn1_beta
        sleep 15
    fi
    #进入项目目录并更新项目
    cd /data/wwwroot/homemoji_vn1_beta
    echo "enter the directory `pwd`";
    git checkout .
    ...
    if [[ $? -eq 0 ]];then
        ...
        git log --oneline | head -10 >> /data/aslog/`date "+%Y%m%d"`/$1.log
        echo "It's Being ready to copy backend web project to folder /data/wwwroot/$1" >> /data/aslog/`date "+%Y%m%d"`/$1.log
        chown -R www:www /data/wwwroot/homemoji_vn1_beta
        ls -lha /data/wwwroot/homemoji_vn1_beta >> /data/aslog/`date "+%Y%m%d"`/$1.log
        sleep 3
    fi
    ...
    sed -i "s/offcialConnect:true/offcialConnect:false/g" /data/wwwroot/homemoji_vn1_beta/moblie/static/js/style.js
    sed -i "s/offcialConnect:true/offcialConnect:false/g" /data/wwwroot/homemoji_vn1_beta/pc/static/js/global.js
    ;;
    esac
fi